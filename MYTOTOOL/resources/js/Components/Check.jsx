...
export function Check({ task }) {
    ...
    function completeTask(e) {
        ...
        Inertia.put(route("task.update", task[0]), { completed: true });
    ...
    return (
        <div className={"completed circle-container relative flex-shrink-0 h-6 self-center"}>
            <button onClick={completeTask} className={(task[3] === 1 ? "bg-green-500 border-green-500" : "") + " circle border-gray-500  border-2 w-6 h-6 rounded-full cursor-pointer transition-colors duration-300 animate-150 ease-in-out "}>
            </button>
        </div>
    )
}
